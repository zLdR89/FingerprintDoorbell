...
void FingerprintManager::setActionConfirmed(bool state)
{
  if (actionConfirmedReset == true)
  {
    if (state == true)
    {
      notifyClients("actionConfirmed is 'on'");
      finger.LEDcontrol(FINGERPRINT_LED_ON, 0, actionColor);
    }
    else
    {
      notifyClients("actionConfirmed is 'off'");
      finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_RED);
    }
  }
  delay(3000);
  setLedRingReady();
  actionConfirmedReset = true;
}
...
void FingerprintManager::configTouchRingActive(uint8_t color, uint8_t sequence, uint8_t scanColor, uint8_t matchColor, uint8_t actionColor)
{
